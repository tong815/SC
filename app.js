...
function createEmptyProgress() {
  ...
  getPracticeTopics().forEach((topic) => {
    topicStats[topic] = { answered: 0, correct: 0, wrong: 0 };
  });
  ...
  return {
    ...
    topicStats,
    gameProgress: {
      playerPosition: { x: mapData.playerStart.x, y: mapData.playerStart.y },
      clearedTowerIds: [],
      keyFragments: [],
...
function renderMap() {
  ...
  mapData.towers.forEach((tower) => {
    ...
    const isCleared = progress.gameProgress.clearedTowerIds.includes(tower.id);
    const isLocked = tower.type === "central" && !progress.gameProgress.centralTowerUnlocked;
...
function renderBlacksmith() {
  ...
  const canForge = collectedFragments >= requiredFragments && !progress.gameProgress.hasCentralTowerKey;
  fragmentCountElement.textContent = `Key fragments: ${collectedFragments} / ${requiredFragments}`;
  forgeKeyButton.disabled = !canForge;
  forgeKeyButton.textContent = progress.gameProgress.hasCentralTowerKey ? "Key Forged" : "Forge Key";
}
...
function handleTowerClick(tower) {
  ...
  if (tower.type === "central" && !progress.gameProgress.centralTowerUnlocked) {
    setMapStatus("The Central Tower is locked. Clear all six outer towers and forge the key first.");
    return;
  }
...
function movePlayerTo(x, y) {
  progress.gameProgress.playerPosition = { x: Math.round(x), y: Math.round(y) };
  renderPlayerPosition();
}

function clearTowerForTopic(topic) {
  const tower = getOuterTowers().find((candidate) => candidate.topic === topic);
  if (!tower || progress.gameProgress.clearedTowerIds.includes(tower.id)) {
    return;
  }
  progress.gameProgress.clearedTowerIds.push(tower.id);
  if (tower.reward?.type === "keyFragment" && !progress.gameProgress.keyFragments.includes(tower.reward.id)) {
    progress.gameProgress.keyFragments.push(tower.reward.id);
  }
...
function forgeCentralTowerKey() {
  const hasAllFragments = getBlacksmithRecipe().input.every((fragmentId) =>
    progress.gameProgress.keyFragments.includes(fragmentId),
  );
  if (!hasAllFragments) {
  ...
  progress.gameProgress.hasCentralTowerKey = true;
  progress.gameProgress.centralTowerUnlocked = true;
...
function updateProgress(question, isCorrect) {
  ...
  if (!progress.topicStats[question.topic]) {
    progress.topicStats[question.topic] = { answered: 0, correct: 0, wrong: 0 };
  }
...
function saveProgress() {
  ...
  const file = new Blob([JSON.stringify(saveData, null, 2)], { type: "application/json" });
  const url = URL.createObjectURL(file);
  const link = document.createElement("a");
  link.href = url;
  link.download = "exam-visualizer-progress.json";
...
function loadProgressFile(event) {
  ...
  reader.addEventListener("load", () => {
    try {
      progress = validateAndNormalizeSaveData(JSON.parse(reader.result));
      renderProgress();
      renderMap();
  ...
  reader.addEventListener("error", () => setProgressMessage("The progress file could not be read."));
...
function normalizeProgress(importedProgress) {
  ...
  correctIds.forEach((questionId) => {
    const question = questionById.get(questionId);
    if (!question || !nextProgress.answeredQuestionIds.includes(questionId)) return;
  ...
  wrongIds.forEach((questionId) => {
    const question = questionById.get(questionId);
    if (!question || !nextProgress.answeredQuestionIds.includes(questionId) || nextProgress.correctQuestionIds.includes(questionId)) return;
...
function normalizeGameProgress(importedGameProgress = {}) {
  ...
  const validFragmentIds = new Set(getOuterTowers().map((tower) => tower.reward?.id).filter(Boolean));
  const position = importedGameProgress.playerPosition || emptyGameProgress.playerPosition;
  const keyFragments = getUniqueStringArray(importedGameProgress.keyFragments).filter((fragmentId) =>
    validFragmentIds.has(fragmentId),
  );
  return {
    playerPosition: {
    ...
    clearedTowerIds: getUniqueStringArray(importedGameProgress.clearedTowerIds).filter((towerId) =>
      validTowerIds.has(towerId),
    ),
    keyFragments,
...
function getQuestionsForTopic(questions, topic) {
  return topic === ALL_QUESTIONS_TOPIC ? questions : questions.filter((question) => question.topic === topic);
}
...
function createQuestionCard(question, questionIndex) {
  ...
  const optionButtons = question.options.map((option) => `
    <button class="option-button" type="button" data-option-id="${option.id}" aria-describedby="feedback-${question.id}">
  ...
  card.innerHTML = `
    ...
    <div class="option-list" role="group" aria-label="Answer choices">${optionButtons}</div>
...
function handleAnswer(question, card, selectedButton) {
  ...
  optionButtons.forEach((button) => {
    ...
    if (button.dataset.optionId === question.answer) button.classList.add("is-correct");
  });
  ...
  feedbackElement.innerHTML = `
    ...
    <p class="feedback-answer"><strong>Correct answer:</strong> ${correctOption.id}. ${correctOption.text}</p>
    <p class="feedback-explanation"><strong>Explanation:</strong> ${question.explanation}</p>
  `;
  setProgressMessage(wasCounted ? "Progress updated." : "This question was already counted in your progress.");
}
...
function showLoadError(error) {
  ...
  errorMessageElement.textContent = "The map or question bank could not be loaded. Please run the project through a local web server and try again.";
  console.error(error);
}
...
async function initializeExam() {
  try {
    [allQuestions, mapData] = await Promise.all([loadJson("questions.json"), loadJson("map.json")]);
